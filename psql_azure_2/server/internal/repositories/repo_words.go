...
func (rt *repoWords) GetAllWords() ([]*models.Word, error) {
	...
	if err != nil {
		appErr := apperrors.GetAllWordsRepoWordsErr.AppendMessage(err)
		rt.log.Error(appErr)
		return nil, appErr
	}
...
func (rt *repoWords) InsertWords(ctx context.Context, words []*models.Word) error {
	for _, word := range words {
		if word == nil {
			appErr := apperrors.InsertWordsErr.AppendMessage("lib == nil")
			rt.log.Error(appErr)
			return appErr
		}
		...
		if tx.Error != nil {
			appErr := apperrors.InsertWordsErr.AppendMessage(tx.Error)
			rt.log.Error(appErr)
			return appErr
		}
		...
		if result.Error != nil {
			appErr := apperrors.InsertWordsErr.AppendMessage(result.Error)
			rt.log.Error(appErr)
			return appErr
		}

		if result.RowsAffected == 0 {
			appErr := apperrors.InsertWordsErr.AppendMessage("no rows affected")
			rt.log.Error(appErr)
			return appErr
		}
		...
		if err := tx.First(createdLib, "id = ?", word.ID).Error; err != nil {
			appErr := apperrors.InsertWordsErr.AppendMessage(err)
			rt.log.Error(appErr)
			return appErr
		}
	}
...
func (rt *repoWords) InsertWord(ctx context.Context, word *models.Word) error {
	...
	result := tx.Create(word)
	if result.Error != nil {
		appErr := apperrors.InsertWordErr.AppendMessage(result.Error)
		rt.log.Error(appErr)
		return appErr
	}
